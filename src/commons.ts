...
export type TTraverser<N extends INode = INode> = (node: N) => N[];
...
export function deepTraverse<N extends INode<N>>(
  nodes: N[],
  traverser?: TTraverser
): N[] {
...
export type TreeVisitor_a<N extends INode<N>> = (
  node: N,
  ancestors: N[]
) => Promise<N[]>;

export async function iterateTree_a<ITNode extends INode<ITNode>>(
  roots: ITNode[],
  visitor: TreeVisitor_a<ITNode>
): Promise<ITNode[]> {
...
export type TreeVisitor<ITNode extends INode<ITNode>> = (
  node: ITNode,
  ancestors: ITNode[]
) => ITNode[];
export function iterateTree<ITNode extends INode<ITNode>>(
  roots: ITNode[],
  visitor: TreeVisitor<ITNode>
): ITNode[] {
  let nodeStack: ITNode[] = [...roots];
  let ancestors: ITNode[] = [];
